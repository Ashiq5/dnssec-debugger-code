...
def _sign(
    ...
    *origin*, a ``dns.name.Name`` or ``None``.  If ``None``, the default, then all
    ...
    if isinstance(rrset, tuple):
        ...
        rrname = rrset[0]
        original_ttl = rrset[1].ttl if not rrsig_ttl else rrsig_ttl
    else:
        rdclass = rrset.rdclass
        rdtype = rrset.rdtype
        rrname = rrset.name
        original_ttl = rrset.ttl if not rrsig_ttl else rrsig_ttl
    ...
    labels = len(rrname) - 1 if not labels else labels
