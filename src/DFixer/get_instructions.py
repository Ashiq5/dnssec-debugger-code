...
def get_instructions(zone_name: str = None, name: str = None, extra_qtypes: str = None):
    try:
        # Step 0: if domain is passed, run dnsviz validation on the domain
        if name:
            if extra_qtypes:
                extra_args = "-R " + extra_qtypes
            else:
                extra_args = ""
            cmd = get_dnsviz_validation_cmd(name, extra_args)
            run_dnsviz_validation(cmd)

        # Step 1: Load validation data from file
        analysis = load_grok()

        # Step 2: identify prevalent misconfigurations in validated data
        ...
        logger.logger.debug("In get instructions, dnskey_map: ", dnskey_map)
        ...
        top_errcode = pick_topologically_first(errcodes, ignored_errcodes)
        if not top_errcode:
            return
